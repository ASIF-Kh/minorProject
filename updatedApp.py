...
from RAG.LLM import initialize_chatbot, create_query_engine, ingestion_pipeline
...
if 'selected_db' not in st.session_state:
    st.session_state.selected_db = "pinecone"  # Default value
...
def main():
    ...
    with st.sidebar:
        ...
        selected_db = st.radio("Choose a database:", ["pinecone", "ChromaDB"])
        ...
        if st.button("Ingest"):
            st.success(f"Data ingestion triggered using **{st.session_state.selected_db}**")
            ingestion_pipeline(st.session_state.selected_db)
        # Ingest Button
        st.write("### Reinitialize System")
        if st.button("Reinitialize"):
            st.success(f"Reinitialize triggered using **{st.session_state.selected_db}**")
            with st.spinner("Initializing system..."):
                index = initialize_chatbot(st.session_state.selected_db)
